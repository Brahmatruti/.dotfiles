...
pacman_install_with_retry() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        if execute "sudo pacman -S --noconfirm $package" "$description"; then
            return 0
        else
...
yay_install_with_retry() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        if execute "yay -S --noconfirm $package" "$description"; then
            return 0
        else
            retry_count=$((retry_count + 1))
            print_warning "Attempt $retry_count failed for $description"
...
pacman_install() {
    ...
    if ! pacman -Q "$package" 2>/dev/null | grep -q "^$package"; then
        print_title "Installing $description..."
        if pacman_install_with_retry "$description" "$package"; then
...
yay_install() {
    ...
    if ! pacman -Q "$package" 2>/dev/null | grep -q "^$package"; then
        print_title "Installing $description..."
        if yay_install_with_retry "$description" "$package"; then
            print_success "$description installed successfully"
        else
            print_error "Failed to install $description"
            handle_package_error "$description" "$package"
